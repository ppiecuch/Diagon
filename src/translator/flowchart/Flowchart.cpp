...
Draw ParseUnmerged(FlowchartParser::ConditionContext* condition, bool is_final) {
  ...
  Draw if_then_else_ = ConnectHorizontally(std::move(if_else_), std::move(then_));
...
Draw Parse(FlowchartParser::ProgramContext* program, bool is_final) {
  Draw out;
  const int n = program->instruction().size();
  for (int i = 0; i < n; ++i) {
    out = MergeBottoms(std::move(out));
    out = ConnectVertically(std::move(out), Parse(program->instruction()[i], is_final && (i == n - 1)));
  }
  return out;
}
